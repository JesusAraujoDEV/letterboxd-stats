# AGENTS.md — letterboxd-stats IA Entry
...
Client-side web app that reads a user's Letterboxd export and turns it into visual film-watching statistics (charts, maps, shareable images). Frontend-only: no backend of its own.

Full specification: [`docs/spec.md`](docs/spec.md) — not written yet.
...
| Components | shadcn/ui over Radix primitives (`src/components/ui`) |
| Styling | Tailwind CSS 3 + tailwindcss-animate, next-themes |
| Data | TanStack Query, Zod, react-hook-form |
| Viz | Recharts, react-simple-maps, html-to-image |
| Tests | Vitest + Testing Library (jsdom) |
| Lint | ESLint 9 (flat config) + typescript-eslint |

Locked decisions (no re-discuss): no backend, no server state store beyond TanStack Query, no Redux, no component library other than shadcn/Radix.

## Architecture rules

- **Layers**: Pages → Hooks → lib (pure logic) → Components. Routing lives in `src/App.tsx`.
- **No business logic in views.** Components render props/state and dispatch events; parsing, aggregation and stats belong in `src/lib` or a hook.
- **Shared state via context/hooks**, not prop drilling through more than two levels (`src/context`).
- **Single source of truth for types**: `src/types` for shared types; Zod schemas at every boundary (parsed CSV/export input above all).
...
src/
  ...
  hooks/           custom hooks
  lib/             pure logic: parsing, aggregation, helpers
...
docs/              agent-facing documentation (see Documentation map)
...
| Role(s) | Human owner |
|---------|-------------|
| Sponsor (approves `docs/briefs/`) | session user (solo project) |
| All roles | session user |
